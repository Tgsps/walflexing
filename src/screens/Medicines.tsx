...
import { tMedName } from '../i18n/content';
...
export default function Medicines() {
  ...
  return (
    <div>
      <ScreenHeader emoji="💊" title={t('medicines.title')} subtitle={t('medicines.subtitle')} />

      <Card className="mb-3 bg-green text-white border-gold">
        <div className="flex items-center justify-between">
          <span className="font-black">{t('medicines.takenToday')}</span>
      ...
      {notificationsSupported() && perm !== 'granted' && (
        <Card className="mb-3">
          <div className="flex items-center gap-2 mb-2">
            ...
            <span className="font-black text-green">{t('medicines.enable')}</span>
          </div>
          {!standalone && (
      ...
      {perm === 'granted' && (
        <p className="text-xs font-bold text-ok mb-3 flex items-center gap-1">
          <Bell size={14} /> {t('medicines.enabledOk')}
        </p>
      )}
      ...
      <ul className="space-y-2">
        {meds.map((m) => {
          ...
          return (
            <li
              key={m.id}
              className={`rounded-2xl border p-3 flex items-center gap-3 transition ${!m.enabled ? 'bg-cream/60 border-line opacity-60' : taken ? 'bg-gold-soft/50 border-gold' : 'bg-card border-line'}`}
            >
              <button
                onClick={() => toggleTaken(m.id)}
                disabled={!m.enabled}
                className={`w-9 h-9 shrink-0 rounded-full border-2 grid place-items-center transition ${taken ? 'bg-green border-green text-white' : 'border-line text-transparent'}`}
                aria-label={t('medicines.takenToday')}
              >
                <Check size={18} strokeWidth={3} />
              </button>
              <span className="text-2xl shrink-0">{m.emoji}</span>
              <div className="flex-1 min-w-0">
                <div className={`font-bold ${taken ? 'line-through text-muted' : 'text-ink'}`}>{tMedName(m.id, t, m.name)}</div>
      ...
      {meds.length === 0 && <p className="text-center text-sm text-muted font-bold py-6">{t('medicines.empty')}</p>}
...
function AddMedicineModal({ onClose }: { onClose: () => void }) {
  ...
  const save = () => {
    if (!name.trim()) return;
    update((d) => {
      d.medicines.push({ id: uid('m'), name: name.trim(), emoji: emoji.trim() || '💊', time, enabled: true });
    });
    onClose();
  };

  return (
    <Modal open onClose={onClose} title={t('medicines.new')}>
      <div className="space-y-3">
        <div className="flex gap-2">
          <input value={emoji} onChange={(e) => setEmoji(e.target.value)} className="field w-16 text-center text-2xl" aria-label="emoji" />
          <input value={name} onChange={(e) => setName(e.target.value)} placeholder={t('medicines.namePh')} className="field flex-1" autoFocus />
        </div>
        <label className="block">
          <span className="text-sm font-bold text-muted">{t('medicines.time')}</span>
